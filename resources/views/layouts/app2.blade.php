            ...
            <li class="nav-item">
                ...
                <div id="collapseBootstrap" class="collapse" aria-labelledby="headingBootstrap"
                    ...
                    <div class="bg-white py-2 collapse-inner rounded">
                        ...
                        <a class="collapse-item" href="{{ route('ajouterTacheForm') }}"> créer un ticket </a>
...
<li class="nav-item">
    ...
    $notificationsAffectation = App\Models\Notification::where('type', 'affectation')
        ->where('vu', 0)
    ...
    $notificationsCreatedAff = App\Models\Notification::where('type', 'createdAff')
        ->where('vu', 0)
        ->whereHas('tache', function ($query) use ($user_id) {
    ...
    $notificationsTerminee = App\Models\Notification::where('type', 'terminée')
        ->where('vu', 0)
        ->whereHas('tache', function ($query) use ($user_id) {
            $query->where('createdBy', $user_id); 
        })
    ...
    $notifications = $notificationsAffectation->merge($notificationsCreatedAff)->merge($notificationsTerminee);
    
    $notifications = $notifications->sortByDesc('date');
    
    $unreadNotificationsCount = $notifications->where('VUFromBouttonNotif', 0)->count();
@endphp
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Handle click event to mark all notifications as read
        ...
        .then(data => {
            if (data.success) {
                ...
                document.querySelectorAll('.notification-item').forEach(function(item) {
                    item.classList.add('read'); // Optional: Add a class to mark notifications visually as read
                });
            } else {
